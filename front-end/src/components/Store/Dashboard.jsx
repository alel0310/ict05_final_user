        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
          {topMenus.map((menu) => (
            <div key={menu.rank} className="flex flex-col items-center p-4 bg-light-gray rounded-lg">
              ...
              <p className="text-sm font-medium text-gray-900">{fmtWonMan(menu.sales)}</p>
            </div>
          ))}
        </div>
      </Card>
          ...
          <TabsContent value="sales" className="mt-6">
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={dailyHourlyData}>
                ...
                <YAxis stroke="#6C757D" />
                <Tooltip formatter={(value) => [fmtWonMan(value), '매출']} />
        ...
        <Card className="p-6 bg-white rounded-xl shadow-sm">
          ...
          <div className="space-y-3">
            {weeklyData.map((day, idx) => (
              <div key={idx} className="flex items-center justify-between p-4 bg-light-gray rounded-lg">
                ...
                <div className="grid grid-cols-3 gap-4 text-right">
                  <div>
                    ...
                    <p className="font-medium text-gray-900">{fmtWonMan(day.sales)}</p>
                  </div>
                  <div>
          ...
          <div className="mt-4 pt-4 border-t border-gray-200">
            <div className="flex items-center justify-between">
              ...
              <div className="grid grid-cols-3 gap-4 text-right">
                <div>
                  <p className="font-semibold text-kpi-red">
                    {fmtWonMan(weeklyData.reduce((s, d) => s + d.sales, 0))}
                  </p>
                </div>
                <div>
                  <p className="font-semibold text-kpi-orange">
                    {weeklyData.reduce((s, d) => s + d.orders, 0)}건
                  </p>
                </div>
                <div>
                  <p className="font-semibold text-kpi-green">
                    {weeklyData.reduce((s, d) => s + d.visitors, 0)}명
                  </p>
                </div>
              </div>
            </div>
            ...
            {monthlyData.map((m, idx) => (
              <div key={idx} className="flex items-center justify-between p-4 bg-light-gray rounded-lg">
                <div className="flex-1">
                  ...
                  <p className="text-xs text-dark-gray">{m.days}일</p>
                </div>
                <div className="grid grid-cols-3 gap-4 text-right">
                  <div>
                    <p className="text-xs text-dark-gray">매출</p>
                    <p className="font-medium text-gray-900">
                      ₩{(Number(m.sales || 0) / 100000000).toFixed(1)}억
                    </p>
                  </div>
                  <div>
                    <p className="text-xs text-dark-gray">주문</p>
                    <p className="font-medium text-gray-900">
                      {(Number(m.orders || 0) / 1000).toFixed(1)}K
                    </p>
                  </div>
                  <div>
                    <p className="text-xs text-dark-gray">방문</p>
                    <p className="font-medium text-gray-900">
                      {(Number(m.visitors || 0) / 1000).toFixed(1)}K
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="grid grid-cols-3 gap-4 text-right">
                ...
                <div>
                  <p className="font-semibold text-kpi-green">
                    {(
                      monthlyData.reduce((s, m) => s + m.visitors, 0) /
                      monthlyData.length /
                      1000
                    ).toFixed(1)}K
                  </p>
